...
def get_mnist_loader(batch_size=16, num_workers=0, train=True):
    ...
    dataset = mnist.MNIST(
        '~/datasets/MNIST', transform=T.Compose(transform), train=train)
...
def get_universal_pwl_fn(num_breakpoints):
    ...
    pwl = maybe_cuda(
        torchpwl.MonoPWL(
            num_features=1, num_breakpoints=num_breakpoints, monotonicity=1))
...
def plot_and_add(ims, pwl, x_start=-7., x_end=7.):
    pwl.cpu()
    ...
    y = pwl(x)
    brk = plt.plot(
        list(pwl.x_positions.squeeze(0)),
        list(pwl(pwl.x_positions.view(-1, 1)).squeeze(1)), 'or')
...
if __name__ == '__main__':
    ...
    opt = torch.optim.SGD(
        params=list(cnn.parameters()) + list(upwl.parameters()),
        lr=0.1,
        momentum=0.5)
    ...
    def update_fn(trainer, batch):
        ...
        trainer.state.add_scalar_data_point(
            'loss', loss.item(), smoothing=0.98)
        trainer.state.add_scalar_data_point('acc', acc.item(), smoothing=0.98)

    st = trainer.STrainer(
        update_fn,
        info_string_components=trainer.DEFAULT_INFO_STRING_COMPONENTS +
        ('acc: {acc:.3f}', ),
        modules=[cnn])
    st.state.acc = 'l'
