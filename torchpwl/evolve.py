...
xa = torch.linspace(-2, 2, steps=10).unsqueeze(0)
c = torch.nn.Sequential(
    Calibrator(keypoints=xa, monotonicity=[-1]),
...
def eval_fn(module):
    ...
    return loss.item() + random.random() * 0.01
...
for e in range(500):
    ...
    opt.zero_grad()
    base_loss = estimate_gradients(module=c, eval_fn=eval_fn, population=500, std=0.001)
    opt.step()
    print(base_loss)

animation.ArtistAnimation(fig, ims, repeat=True).save(
        'xx.gif', writer='pillow')
import torch.jit
